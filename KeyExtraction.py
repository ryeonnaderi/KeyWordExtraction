...
def extract_keywords_pytextrank(text, num_keywords=20):
    doc = nlp(text)

    # Remove keywords that contain special characters (Getting rid of it grabbing = as a keyword)
    keywords = [
        (phrase.text.strip().lower(), phrase.rank)
        for phrase in doc._.phrases
        if not any(char in string.punctuation for char in phrase.text)  # Remove if contains punctuation
    ]

    return keywords[:num_keywords]

def evaluate_keywords(reference, predicted):
    y_true = [1 if kw in reference else 0 for kw in predicted]
    y_pred = [1] * len(predicted)  # All extracted keywords are considered 'positive'
